...
6. Remaining files via `loadraw` (Krill, `$01=$35`, no `IOINIT`) or KERNAL SETNAM/SETLFS/LOAD: `locode`, `scr`, `sfx`, `wpn`, `itm`, `bmp`, `sqt`, `texlo`, `paint`, `tab`, `col`. **LOCODE** overwrites MENU at `$0900`.
...
| KB | Range | Contents |
|----|-------|----------|
| 0 | `$0000`–`$03FF` | CPU port, game ZP, under-stack BSS, page-2 KERNAL scrap during LOAD, cassette BSS `$033C`–`$03E8`. |
...
| 2–13 | `$0900`–`$24AE` | **`locode`** (7087 B). Free `$24AF`–`$37FF` (~4945 B) to SQTAB. |
| 14–15 | `$3800`–`$3FFF` | **`sqt`** Judd tables (2K). |
| 16–17 | `$4000`–`$47E7` | **`scr`**: matrix A `$4000`, 24-byte sprite-ptr pad, matrix B `$4400`. |
| 18–19 | `$4800`–`$4C5E` | **`sfx`** (1119 B). Slack `$4C60`–`$4DFF`. **Krill `loadraw` `$4E00`–`$4EEB`** on **`wolf64-krill.d64`** only (reserved hole on both disks). |
...
| `$033C`–`$03E8` | Cassette BSS (player/doors/AI scratch; not in locode PRG). Limit `$03FC`. |
| `$0400`–`$07D2` | Render tables (`tab`) |
| `$0801`–`$08A9` | Low BSS overlay (was boot) |
| `$08C0` | `REBOOT_STUB` |
| `$08FD`–`$08FF` | Menu selectors |
| `$0900`–`$24AE` | Locode (game code; no enemy modules) |
| `$24AF`–`$37FF` | Free (runtime). MENU used this up to `$3633` before locode loaded. |
| `$3800`–`$3FFF` | Judd SQTAB1–4 |
| `$4000`–`$47E7` | VIC matrices A/B |
| `$4800`–`$4C5E` | PC SFX |
| `$4C60`–`$4DFF` | Slack after SFX / menu sprites |
...
| Play | locode | locode (if `end_locode` grows) | `sqt` | `scr` | `sfx` | `bmp` | `paint` |
...
| `locode` | slice | `$0900` | `$0900`–`$24AE` |
| `scr` | slice | `$4000` | `$4000`–`$47E7` |
| `sfx` | slice | `$4800` | `$4800`–`$4C5E` |
...
## Cassette-buffer BSS (`$033C`–`$03E8`)

Locode runtime; zero-filled once in `game_start`. Disk `LOAD` does not use the tape buffer. Limit `TAPE_BSS_END` `$03FC`.

Player, doors (8 slots), enemy-draw scratch, `turn_acc`, mouse/frame/score. `end_tape_bss` = `$03E9` this build (~173 B).
...
IRQ/NMI save `$01`, set `$35`, ack, restore. Hardware vectors `$FFFE` / `$FFFA` (KERNAL out). In-play `LoadLevel` `IOINIT`s and must re-run `init_vic` / `input_irq_init` / `play_sound_init`.
...
| `$24AF`–`$37FF` | ~4945 | After locode; MENU occupied it until locode loaded |
| `$4C60`–`$4DFF` | ~416 | After SFX; SFX growth room before Krill hole |
...
Assemble overlap errors: locode vs `SQTAB1`, SFX vs `WPN_SPRITES`, painters+SoA vs `ENEMY_BASE`, enemy vs `MAP`, tape BSS vs `$03FC`, stack BSS vs `$01D0`. Locode/enemy free bytes are ACME `!warn` at assemble.
