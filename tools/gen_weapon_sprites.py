...
Outputs:
  src/weapons/wpn_data.asm    sprite blobs (linked at WPN_MASTER $9000)
  src/weapons/wpn_tables.asm  colours, XY, master src addrs; stage copy at runtime
"""
...
def main() -> int:
    ...
    # Master lives at WPN_MASTER ($9000); stage is 8×64 under the hole.
    if nbytes > 0x1000:
        raise SystemExit(f"weapon sprites ${nbytes:04X} exceed hole to BITMAP")

    data_lines = [
        "; Auto-generated by tools/gen_weapon_sprites.py — do not edit",
        "; Linked at WPN_MASTER ($9000); active frames copied to WPN_STAGE.",
        "!zone wpn_data",
        "",
    ]
    ...
    fx: list[int] = []
    fy: list[int] = []
    flash_ptr_labels: list[str | None] = []
    flash2_ptr_labels: list[str | None] = []
    for p in packed:
    ...
    def addr_lo(label: str | None) -> str:
        return "0" if label is None else f"<{label}"

    def addr_hi(label: str | None) -> str:
        return "0" if label is None else f">{label}"

    body_labels = [f"{p['spec']['name']}_spr0" for p in packed]

    tbl = [
        "; Auto-generated by tools/gen_weapon_sprites.py — do not edit",
        "; Master labels at WPN_MASTER; runtime copies into WPN_STAGE.",
        "; Flush+centered sprite XY; runtime adds pose delta.",
        ...
        bcol(en_fire),
        "wpn_body_src_l",
        "\t!byte " + ", ".join(f"<{lab}" for lab in body_labels),
        "wpn_body_src_h",
        "\t!byte " + ", ".join(f">{lab}" for lab in body_labels),
        "wpn_flash_src0_l",
        "\t!byte " + ", ".join(addr_lo(lab) for lab in flash_ptr_labels),
        "wpn_flash_src0_h",
        "\t!byte " + ", ".join(addr_hi(lab) for lab in flash_ptr_labels),
        "wpn_flash_src1_l",
        "\t!byte " + ", ".join(addr_lo(lab) for lab in flash2_ptr_labels),
        "wpn_flash_src1_h",
        "\t!byte " + ", ".join(addr_hi(lab) for lab in flash2_ptr_labels),
        "",
